...
<div class="row">
	<div class="col-lg-12">
		<ol class="breadcrumb">
			<li class="active">
				<i class="fa fa-dashboard"></i>Dashboard / View Sale
			</li>
		</ol>
	</div>
</div>
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">
					<i class="fa fa-money fa-fw"></i>View Sale
				</h3>
				<hr>
				...
				<?php 
					if(isset($_POST['search'])){
						...
						//$get_sale="select order_id, customer_id, invoice_no, order_date, SUM(due_amount) AS due_amount1 from customer_order where order_date between '$fdate' and '$tdate' order by order_id";
						$get_sale="select invoice_no, SUM(due_amount) AS due_amount1 from customer_order where order_date between '$fdate' and '$tdate' Group BY invoice_no order by invoice_no";
						
						
						
					}else{
						$get_sale="select order_id, customer_id, invoice_no, order_date, SUM(due_amount) AS due_amount1 from customer_order where order_status='Delivered'  group by invoice_no order by order_date";
					}
				?>
			</div>
				...
				<div class="table-responsive">
					<table class="table table-borderd table-hover table-striped">
						...
						<?php 
							...
							$run_order=mysqli_query($con,$get_sale);
							while($row=mysqli_fetch_array($run_order)){
								$order_id=$row['order_id'];
								$c_id=$row['customer_id'];
								$invoice_no=$row['invoice_no'];
								$order_date=$row['order_date'];
								$due_amount=$row['due_amount1'];
								//$order_status=$row['order_status'];
								$tamount=$tamount+$due_amount;
								$i++				
